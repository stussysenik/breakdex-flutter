...
void main() {
  group('Card count sync — matrix vs session items', () {
    ...
    test('Learning count matches filtered session items with state=Learning',
        () async {
      ...
      // Query matrix
      final matrix =
          await container.read(reviewStateMatrixProvider.future);
      ...
      final sessionItems =
          await container.read(filteredReviewSessionItemsProvider.future);
    ...
    test('FSRS card state change reflected in both providers', () async {
      ...
      await seedFsrsCard(db,
          entityId: 'move-1', entityType: 'move', fsrsState: 1);
      await settleStreams();
