...
| **Face Law** | **Six checkable rules — chrome essentialism stated so a diff answers yes/no** | Decided 2026-07-30, specced 2026-08-02. (1) **One frame** — every screen is `AppScreen`; a raw `Scaffold`/`AppBar`/`SliverAppBar` under `lib/features/` fails the gate, not review. (2) **One primary action per content band** — a second one demotes or the surface splits. (3) **Monochrome carries, colour marks state** — colour is a signal, never decoration. (4) **A control shows its value** — a chrome control that renders only its noun where a current value exists is a violation. (5) **A new chrome control displaces one** — or the change ledger says why the count grew; control count is countable from the diff. (6) **Density from tokens only** — chrome dimensions resolve from `AppLayout`/TOKENS.md, never a literal. Adjectival guidance ("clean", "minimal") is banned from the doctrine: it is not answerable. Spec: `enforce-face-law-conformance`. |
...
| **Functional visual** | **The working visual is the primary deliverable — present it, don't narrate it** | Decided 2026-08-10. A feature is not done when it compiles; it is done when the owner can *see* it working and judge the look. Build → present the running surface (web preview / screenshot / artifact) → iterate on the visual. Code that ships with no one having seen it is unfinished. **Craft bar:** the visual quality a design lead (Ivan Zhao / Notion) would sign off — intentional spacing, restrained color, composited motion, nothing placeholder. Demo-grade facade is a defect, not a milestone. |
