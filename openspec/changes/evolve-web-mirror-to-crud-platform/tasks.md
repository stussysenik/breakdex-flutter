...
- [ ] 0.3 Confirm which account the phone currently syncs to; if not `senik456@gmail.com`, plan the non-destructive media migration (per `identity-centralization`).
- [~] 0.4 Add `senik456@gmail.com` to the web owner allowlist; verify sign-in resolves the intended Drive. — Allowlist DONE 2026-06-17 (`NEXT_PUBLIC_OWNER_ALLOWLIST=itsmxzou@gmail.com,senik456@gmail.com`; kept itsmxzou additively so the account currently holding data isn't locked out). Sign-in verification + "which Drive has the data" is owner-run (depends on 0.3) — and senik456 must also be a **test user / published** on the Google Cloud OAuth consent screen, else Google blocks it before the allowlist applies.
