...
## Phase 5 — Owner-driven device proof (dedicated session — NOT auto-driven)
- [ ] 5.1 Owner: Google-sign-in on the phone (canonical identity).
- [ ] 5.2 Provision (auto on first login) → verify per-entity row counts match local
      library via server-side `curl` / `get_app_logs`.
- [ ] 5.3 Second surface (web / other device) signs in same identity → same data appears.
- [ ] 5.4 CRUD on either surface propagates within a pull cycle (argent acceptance:
      all devices up, one truth, CRUD works).
