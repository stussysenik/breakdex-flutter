# Developer Docs — Engineering Manual

> Extends the `developer-docs` capability introduced by
> `add-web-authoring-and-lifecycle-studio` (which owns rendering/hosting). This delta owns
> the manual's content, source location, and standards role.
