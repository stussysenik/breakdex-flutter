...
- [x] 2.1 StatusTag widget: current word + ▾; tap → four-word picker (idea/attempting/landed/clean); writes via 1.7
- [x] 2.2 Detail header: `‹ COMBOS` breadcrumb, `titleLarge` name, transition chain line (secondary, wraps)
...
- [x] 2.4 JournalList: 56/16/fluid grid, fluid type by length (14/16), `kind='status'` rows muted style, video refs render name+size with resolver-miss fallback
- [x] 2.5 JotComposer pinned bottom: text field ("Jot it down…"), `+ video`, send (44dp circle, accent); writes immutable jot
- [x] 2.6 LibraryVideoPickerSheet: sections "THIS COMBO'S MOVES" (with usage counts) and "RECENT TAKES"; instant link (no copy); "Import new from Photos…" last row
- [x] 2.7 ⋯ action sheet: Plan for a day…, Duplicate, Edit, Share, Save to Album, Delete (destructive style)
- [x] 2.8 Duplicate: clones combo+combo_moves as new `idea` combo, seeds journal with `kind='duplicate'` provenance row; test
- [x] 2.9 Remove NOTES section from combo detail (jots supersede); migrate nothing — `combos.notes` stays readable via Edit screen
- [ ] 2.10 `flutter analyze` + widget tests for 2.1/2.4/2.5 green

## Phase 3 — Three tabs (Library · Planned · Calendar)

- [x] 3.1 CombosScreen tab host with `AppSegmentedControl` (Library | Planned | Calendar); FAB "+" on Library only
- [x] 3.2 LibraryView: month-grouped rows (name, 2-line transition chain, tag, mono stamp); stream-driven
- [x] 3.3 PlannedView: progress strip (last session / this week / landed — all ledger-derived), numbered queue with ▲▼ reorder persisting `position`, one-line *why* from latest jot/status
- [x] 3.4 "Plan a combo" primary button on PlannedView → picker: existing combo or "New combo…" (create-then-plan)
- [x] 3.5 ComboCalendarView: reuse PracticeCalendarView pattern; past heat from rollup, future days show dashed ring + plan dots; legend
- [x] 3.6 Day tap: past → that day's combos+facts; future → planned list + "+ Plan"
- [x] 3.7 Create flow: new combo born `status='idea'`; success affordance offers "Plan it?"
- [x] 3.8 Route swap: new CombosScreen replaces old combo list; old screen deleted (lib/features/combo_list removed 2026-06-11)
...
> 2026-06-11: Phases 2–3 completed/corrected via `tighten-combo-journey-and-review-polish` (see that change for the plan-flow fix, calendar visibility, beat grid redesign, and Plan-it affordance). 2.3 uses BeatGrid instead of ComboStepLine on combo detail. 2.10/3.9 verification rolled into the polish change's Phase 6/7.
