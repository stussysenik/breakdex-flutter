...
A "Lab" tab is added as the 5th bottom navigation item with a science/flask icon. Tapping it navigates to /lab.
...
The Lab screen supports two views — List and Board — toggled via AppSegmentedControl.
...
List view shows lab cards sorted by updatedAt DESC with status pill, progress bar, and metadata (attempt count, days since creation).
...
Board view groups labs into 4 horizontal-scroll columns by status: Idea, Attempting, Landed, Clean.
...
A persistent text input at the top allows quick daily log entries, optionally attached to a lab.
...
Users can create new labs with a name and type (project or set).
