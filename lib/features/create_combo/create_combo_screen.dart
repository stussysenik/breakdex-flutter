...
class _CreateComboScreenState extends ConsumerState<CreateComboScreen> {
  ...
  void initState() {
    ...
    DiagnosticsLog.info(
      'CreateCombo',
      'initState isEditing=${widget.isEditing} comboId=${widget.comboId ?? "null"}',
    );
    if (widget.isEditing) {
      _loadExistingCombo();
    }
  ...
  Future<void> _loadExistingCombo() async {
    ...
    try {
      final combo = await ref
          ...
          .getById(widget.comboId!);
      final movesWithDetail = await ref
          .read(comboRepositoryProvider)
  ...
  Future<void> _saveCombo() async {
    ...
    try {
      final companion = CombosCompanion(id: Value(comboId), name: Value(name));
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Failed to save combo: $e')));
      }
    }
  }
            ...
            Padding(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  prefixIcon: const AppIconView(AppIcon.search, size: 20),
                  isDense: true,
                  filled: true,
                ...
                data: (final allMoves) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (final context, final index) {
                      ...
                      final isPicked = _selected.any(
                        (final m) => m.id == move.id,
                      );
                      final isAlreadyInCombo = widget.alreadySelectedIds
                          .contains(move.id);

                      return ListTile(
                        onTap: isAlreadyInCombo
                            ? null
                            : () {
                                setState(() {
                                  if (isPicked) {
                        ...
                        title: Text(
                          move.name,
                          style: TextStyle(
                            color: isAlreadyInCombo
                                ? colorScheme.outline
                                : colorScheme.onSurface,
                          ),
                        ),
                        subtitle: Text(move.category),
                        trailing: isAlreadyInCombo
                            ? const Text(
                                'In Combo',
