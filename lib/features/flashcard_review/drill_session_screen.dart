...
/// The immersive drill session: one card at a time, filling the whole viewport.
///
/// Frameless on purpose — the card *is* the surface, so there is no header to
/// say where you are and no band to tap away from it. The way out is the card's
/// own end control, which asks first. Mounted only while
/// `reviewSessionActiveProvider` is true, so ending a session unmounts this
/// widget and its state goes with it.
class DrillSessionScreen extends ConsumerStatefulWidget {
  const DrillSessionScreen({super.key});
...
class _DrillSessionScreenState extends ConsumerState<DrillSessionScreen>
  ...
  /// Re-fetches the session after it was away — a card rated on a detail
  /// screen, or an app resumed from the background, changes what is due.
  void _reloadSession() {
    if (!mounted) return;
  ...
  @override
  void didPopNext() => _reloadSession();
