...
/// entry action hooks.
final class MoveDetailMachine
    extends Machine<MoveDetailState, MoveDetailEvent> {
  ...
  MoveDetailState? transition(MoveDetailState s, MoveDetailEvent e) {
    return switch ((s, e)) {
      ...
      (Idle(), VideoPicked(localPath: final p, originalFileName: final n)) =>
      ...
      (Idle(), TapDeleteLog(entryId: final entryId)) => ConfirmingDeleteLog(
        s.move,
        entryId: entryId,
      ),
      ...
      // ── Renaming flow ──
      (Renaming(), UpdateDraft(name: final n)) => Renaming(
        s.move,
        draftName: n,
      ),
      (Renaming(), Cancel()) => Idle(s.move),
      (Renaming(), SaveName(name: final n)) => ValidatingName(
        s.move,
        candidateName: n,
      ),

      (ValidatingName(candidateName: final cn), NameAvailable()) => SavingName(
        s.move,
        newName: cn,
      ),
      (ValidatingName(candidateName: final cn), NameTaken()) => NameConflict(
        s.move,
        conflictingName: cn,
      ),

      (NameConflict(), UpdateDraft(name: final n)) => Renaming(
        s.move,
        draftName: n,
      ),
      ...
      (ChangingState(), SaveState(learningState: final st)) => SavingState(
        s.move,
        newState: st,
      ),
      ...
      (ChangingCategory(), SaveCategory(category: final c)) => SavingCategory(
        s.move,
        newCategory: c,
      ),
      ...
      (ChangingCount(), SaveCount(count: final c)) => SavingCount(
        s.move,
        newCount: c,
      ),
      ...
      (Deleting(), DeleteFailed(error: final e)) => ErrorState(s.move, message: e),
      ...
      (
        PickingVideo(),
        VideoPicked(localPath: final p, originalFileName: final n),
      ) =>
        SavingVideo(s.move, localPath: p, originalFileName: n),
      ...
      (AddingLog(), SaveLogBody(body: final b)) => SavingLog(s.move, body: b),
      (SavingLog(), SaveSucceeded(move: final m)) => Idle(m),
      (SavingLog(), SaveFailed()) => Idle(s.move),

      (ConfirmingDeleteLog(entryId: final _), Cancel()) => Idle(s.move),
      (ConfirmingDeleteLog(entryId: final eid), Confirm()) => DeletingLog(
        s.move,
        entryId: eid,
      ),
      ...
      // ── Notes inline ──
      (NotesDirty(), UpdateNotes(text: final t)) => NotesDirty(
        s.move,
        draftText: t,
      ),
