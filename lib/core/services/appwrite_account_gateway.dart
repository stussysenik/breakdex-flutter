...
class AppwriteAccountSdkGateway implements AppwriteAccountGateway {
  ...
  Future<void> createGoogleSession({final List<String> scopes = const []}) async {
    try {
      // `success` is left null: the SDK auto-derives the callback scheme
      // `appwrite-callback-<projectId>` — the scheme 0.2 registered in both
      // iOS plists + AndroidManifest. Empty scopes ⇒ null (identity-only
      // session; the Drive token is minted separately by google_sign_in, 3.3).
