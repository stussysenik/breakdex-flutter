...
class AssetManifestDao extends DatabaseAccessor<AppDatabase>
  ...
  Stream<List<AssetManifestData>> watchAll() =>
      (select(assetManifest)
            ..orderBy([(t) => OrderingTerm.desc(t.importedAt)]))
          .watch();

  Future<List<AssetManifestData>> getAll() =>
      (select(assetManifest)
            ..orderBy([(t) => OrderingTerm.desc(t.importedAt)]))
          .get();

  Future<AssetManifestData?> getByHash(String contentHash) =>
      (select(assetManifest)
            ..where((t) => t.contentHash.equals(contentHash)))
          .getSingleOrNull();

  Stream<AssetManifestData?> watchByHash(String contentHash) =>
      (select(assetManifest)
            ..where((t) => t.contentHash.equals(contentHash)))
          .watchSingleOrNull();
  ...
  Future<List<AssetManifestData>> getStaleVerifications(
    Duration maxAge,
  ) async {
    final cutoff = DateTime.now().subtract(maxAge);
    return (select(assetManifest)
          ..where((t) =>
              t.deletedAt.isNull() &
  ...
  Future<void> softDelete(String contentHash, String reason) =>
      (update(assetManifest)
            ..where((t) => t.contentHash.equals(contentHash)))
          .write(AssetManifestCompanion(
  ...
  Future<void> updateCopyCount(String contentHash) async {
    final copies = await (select(assetCopies)
          ..where((t) =>
  ...
  Future<void> hardDelete(String contentHash) =>
      (delete(assetManifest)
            ..where((t) => t.contentHash.equals(contentHash)))
          .go();
}
