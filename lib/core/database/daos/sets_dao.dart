...
class SetsDao extends DatabaseAccessor<AppDatabase> with _$SetsDaoMixin {
  ...
  Future<String> createSet(SetsCompanion entry) {
    return into(sets).insertReturning(entry).then((row) => row.id);
  }

  Future<bool> updateSet(SetsCompanion entry) {
    return update(sets).replace(entry);
  }

  Future<int> deleteSet(String id) {
    return (delete(sets)..where((t) => t.id.equals(id))).go();
  }

  Future<BreakdexSet> getById(String id) {
    return (select(sets)..where((t) => t.id.equals(id))).getSingle();
  }
  ...
  Stream<BreakdexSet> watchById(String id) {
    return (select(sets)..where((t) => t.id.equals(id))).watchSingle();
  }

  Future<void> addSetItem(SetItemsCompanion entry) async {
    await into(setItems).insertOnConflictUpdate(entry);
  }

  Future<void> removeSetItem(String id) async {
    final item = await (select(setItems)..where((t) => t.id.equals(id))).getSingle();
    await (delete(setItems)..where((t) => t.id.equals(id))).go();
    await _reindexSetItems(item.setId);
  }

  Future<void> reorderSetItem(String itemId, int newPosition) async {
    final item = await (select(setItems)..where((t) => t.id.equals(itemId))).getSingle();
    await (update(setItems)..where((t) => t.id.equals(itemId))).write(
      SetItemsCompanion(position: Value(newPosition)),
    );
    await _reindexSetItems(item.setId);
  }

  Stream<List<SetItem>> watchSetItems(String setId) {
  ...
  Future<bool> validateNoCycle(String setId, String childSetId) async {
    ...
    while (queue.isNotEmpty) {
      ...
      final parentItems = await (select(setItems)
        ..where((t) => t.itemId.equals(current) & t.itemType.equals('set')))
          .get();
    ...
    while (descQueue.isNotEmpty) {
      ...
      final childItems = await (select(setItems)
        ..where((t) => t.setId.equals(current) & t.itemType.equals('set')))
          .get();
  ...
  Future<int> depth(String setId) async {
    var maxDepth = 0;
    final visited = <String>{setId};
  ...
  Future<void> _reindexSetItems(String setId) async {
    ...
    for (var i = 0; i < items.length; i++) {
      if (items[i].position != i) {
        await (update(setItems)..where((t) => t.id.equals(items[i].id))).write(
